...
def get_logo_svg():
    """Generate RiskShield Zimbabwe SVG Logo"""
    svg = """
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200" width="120" height="120">
      ...
      <path d="M100 10 L175 45 L175 110 Q175 165 100 190 Q25 165 25 110 L25 45 Z"
            fill="url(#shieldGrad)" stroke="#FFD700" stroke-width="3"/>
      ...
      <text x="100" y="115" font-family="Georgia,serif" font-size="72" font-weight="bold"
            ...
            style="filter:drop-shadow(0 2px 4px rgba(0,0,0,0.4))">R</text>
      <!-- Gold star at top -->
...
# ==================== COMPLETE ZIMBABWE CURRENCY HISTORY ====================
...
def get_complete_currency_history():
    return pd.DataFrame({
        ...
        'Official_Rate_USD': [
            1.00, 1.00, 2.50, 15.00, 80.00,
            200.00, 1500.00, 8000.00, 13500.00, 26.90
        ],
        'Parallel_Rate_USD': [
            1.00, 1.20, 3.50, 25.00, 150.00,
            400.00, 3000.00, 15000.00, 25000.00, 33.50
        ],
        'Inflation_Rate': [
            5.0, 2.5, 50.0, 150.0, 500.0,
            350.0, 800.0, 600.0, 150.0, 8.5
        ],
        'Key_Event': [
...
def fetch_rbz_data():
    """Current RBZ official data as of March 2026"""
    # Calculate days since ZiG launch (April 5, 2024)
    ...
    return {
        ...
        'zig_introduction_date': '2024-04-05',
...
def get_historical_rate_for_date(target_date):
    ...
    if target < pd.Timestamp('2014-01-01'):
        return 1.0, 'USD'
    elif target < pd.Timestamp('2016-01-01'):
        return 1.0, 'Bond'
    elif target < pd.Timestamp('2018-01-01'):
        return 2.5, 'Bond'
    elif target < pd.Timestamp('2019-01-01'):
        return 15.0, 'RTGS'
    elif target < pd.Timestamp('2020-01-01'):
        return 80.0, 'RTGS'
    elif target < pd.Timestamp('2022-01-01'):
        return 200.0, 'ZWL'
    elif target < pd.Timestamp('2023-01-01'):
        return 1500.0, 'ZWL'
    elif target < pd.Timestamp('2024-04-01'):
        return 8000.0, 'ZWL'
    elif target < pd.Timestamp('2025-01-01'):
        return 13500.0, 'ZiG'
    else:
...
<style>
    ...
    .main-header {
        ...
        text-align: center;
        color: white;
        box-shadow: 0 8px 32px rgba(0,100,0,0.35);
        position: relative;
        overflow: hidden;
    }
    .main-header::before {
...
with col_title:
    ...
    <div class="main-header">
        ...
        <p style="font-size:0.9rem; opacity:0.8;">USD → Bond → RTGS → ZWL → ZiG &nbsp;|&nbsp; Auto-updating with RBZ Data &nbsp;|&nbsp; v7.0</p>
    </div>
    """, unsafe_allow_html=True)

if data_updated:
    st.success(f"🔄 RBZ Data Auto-Updated: {datetime.now().strftime('%H:%M:%S')}")
...
<div class="rbz-banner">
    ...
    <span style="font-size:0.88em;">
        ✅ ZiG Era: <b>{rates['days_stable']}</b> days of stability &nbsp;|&nbsp;
        Inflation: <b>{rates['inflation']}%</b> (Single Digit) &nbsp;|&nbsp;
        1 USD = <b>{rates['official_rate']:.2f} ZiG</b> (Official)
    </span>
</div>
""", unsafe_allow_html=True)
...
with st.sidebar:
    st.markdown(get_logo_svg(), unsafe_allow_html=True)
    st.markdown("## 🛡️ RiskShield Zimbabwe")
...
with st.expander("📜 COMPLETE ZIMBABWE CURRENCY HISTORY (2009–2026)", expanded=False):
    ...
    colors = ['#9e9e9e', '#ff9800', '#f44336', '#e91e63', '#e91e63',
              '#e91e63', '#e91e63', '#ff9800', '#4caf50', '#2e7d32']
    for idx, row in history_df.iterrows():
        ...
        if is_current:
            ...
            <div class="current-era-box">
                <strong>✅ CURRENT ERA: {row['Period']}</strong> — {row['Currency']}<br>
                <span style="font-size:0.9em;">
                    Official: 1 USD = {row['Official_Rate_USD']:.2f} ZiG &nbsp;|&nbsp;
                    Parallel: 1 USD = {row['Parallel_Rate_USD']:.2f} ZiG &nbsp;|&nbsp;
                    Inflation: {row['Inflation_Rate']}%
                </span><br>
                <em style="font-size:0.85em;">{row['Key_Event']}</em>
            </div>
            """, unsafe_allow_html=True)
        else:
            st.markdown(f"""
            <div class="era-box" style="border-left:4px solid {color};">
                <strong>{row['Period']}:</strong> {row['Currency']} &nbsp;|&nbsp;
                1 USD = {row['Official_Rate_USD']:.0f} {row['Currency'].split()[0]} &nbsp;|&nbsp;
                Inflation: {row['Inflation_Rate']}% &nbsp;|&nbsp;
                <em>{row['Key_Event']}</em>
            </div>
            """, unsafe_allow_html=True)
...
with tab1:
    ...
    with col1:
        ...
        with st.form("client_reg_form"):
            ...
            preferred_currency = st.selectbox("Preferred Currency", ['ZiG', 'USD'])
            ...
            if submitted:
                if name and reg_number and phone and address:
                    ...
                    st.session_state.clients[cid] = {
                        ...
                        'Address': address, 'Preferred_Currency': preferred_currency,
    ...
    with col2:
        ...
        if st.session_state.clients:
            ...
            st.info("No clients registered yet. Register your first client.")

# ==================== TAB 2: ASSET INSURANCE ====================
with tab2:
    st.header("📦 Asset Insurance")
    if not st.session_state.clients:
        st.warning("⚠️ Please register a client first (Tab 1).")
    else:
        col1, col2 = st.columns([1, 1])
        with col1:
            ...
            with st.form("asset_form"):
                ...
                if is_historical:
                    ...
                    st.info(f"**Original:** {hist_amt:,.0f} {hist_curr} ({hist_date_a})\n\n"
                ...
                # Premium calculation
                base_rate = 0.025
                inflation_loading = rates['inflation'] / 10000
                annual_prem = value_zig * (base_rate + inflation_loading)
                ...
                econ_factor = rates['inflation'] / 1000
                lapsing_rate = rbz_class_info['max_lapsing'] + econ_factor

                st.success(f"**Monthly Premium:** ZiG {monthly_prem:,.2f} (≈ ${monthly_prem_usd:,.2f})\n\n"
        ...
        with col2:
            ...
            if not st.session_state.assets.empty:
                for idx, asset in st.session_state.assets.iterrows():
                    ...
                    tag = "📜" if asset['Original_Currency'] != 'ZiG' else "🆕"
...
with tab3:
    ...
    if st.session_state.assets.empty:
        st.warning("No policies available. Issue a policy first (Tab 2).")
    else:
        col1, col2 = st.columns([1, 1])
        with col1:
                ...
                with st.form("pay_form"):
                    ...
                    if st.form_submit_button("💳 Record Payment"):
                        new_pay = pd.DataFrame([{
                            'Date': pay_date, 'Client_ID': asset['Client_ID'],
                            'Asset_ID': aid, 'Payment_Amount': zig_amt,
...
with tab4:
    ...
    with col1:
        ...
        if not st.session_state.assets.empty:
            ...
            for _, a in st.session_state.assets.iterrows():
                cn = st.session_state.clients.get(a['Client_ID'], {}).get('Name', 'Unknown')
                asset_opts[f"{cn} – {a['Asset_Description']} (ZiG {a['Asset_Value']:,.0f})"] = a['Asset_ID']
            sel = st.selectbox("Select Asset", list(asset_opts.keys()), key="claim_asset_sel")
            if sel:
                aid = asset_opts[sel]
                asset = st.session_state.assets[st.session_state.assets['Asset_ID'] == aid].iloc[0]
                with st.form("claim_form"):
                    ...
                    try:
                        months = (date.today() - pd.to_datetime(asset['Inception_Date']).date()).days / 30
                    except:
                        months = 0
                    inflation_factor = (1 + (rates['inflation'] / 100 / 12)) ** months
                    base_claim = asset['Asset_Value'] * (damage_pct / 100) * sev_factors[severity]
                    claim_zig = base_claim * inflation_factor
                    claim_usd = claim_zig / rates['official_rate']

                    st.info(f"**Base Claim:** ZiG {base_claim:,.2f}\n\n"
                    ...
                    if st.form_submit_button("📋 Submit Claim"):
                        ...
                        new_claim = pd.DataFrame([{
                            'Claim_ID': cid_new, 'Client_ID': asset['Client_ID'],
                            'Asset_ID': aid, 'Claim_Date': claim_date,
    ...
    with col2:
        ...
        if not st.session_state.claims.empty:
            ...
            for idx, claim in st.session_state.claims.iterrows():
                cn = st.session_state.clients.get(claim['Client_ID'], {}).get('Name', 'Unknown')
                sc = sev_colors.get(claim['Severity_Level'], '#6c757d')
                st.markdown(f"""
                <div style="border-left:4px solid {sc}; padding:0.6rem 1rem; margin:0.4rem 0;
                            background:#fafafa; border-radius:0 0.4rem 0.4rem 0;">
                    <strong>{claim['Claim_ID']}</strong> — {cn}<br>
                    <small>ZiG {claim['Claim_Amount']:,.2f} (${claim['Claim_Amount_USD']:,.2f})
                    &nbsp;|&nbsp; <b>{claim['Severity_Level']}</b>
                    &nbsp;|&nbsp; {claim['Status']}</small>
                </div>
                """, unsafe_allow_html=True)
            total_zig = st.session_state.claims['Claim_Amount'].sum()
            total_usd = st.session_state.claims['Claim_Amount_USD'].sum()
            col_s1, col_s2 = st.columns(2)
            col_s1.metric("Total Claims (ZiG)", f"ZiG {total_zig:,.2f}")
            col_s2.metric("Total Claims (USD)", f"${total_usd:,.2f}")
        else:
...
with tab5:
    ...
    with col1:
        ...
        hist = st.session_state.rbz_historical
        ax.plot(hist['Date'], hist['Inflation_Rate'], 'b-', linewidth=2.5, marker='o', markersize=3)
        ax.axhline(y=rates['inflation'], color='green', linestyle='--',
                   label=f"Current: {rates['inflation']}%", linewidth=1.5)
        ax.fill_between(hist['Date'], hist['Inflation_Rate'], alpha=0.1, color='blue')
        ax.set_xlabel('Date')
        ...
        st.pyplot(fig)
        plt.close()
        st.metric("Inflation Reduction", f"{rates['inflation']}% (currently)",
                  delta=f"From peak 800%+ → {rates['inflation']}%")

    with col2:
        st.subheader("Exchange Rate Stability")
        fig2, ax2 = plt.subplots(figsize=(8, 4))
        # Only show post-rebase (clean) data
        post = hist[hist['Currency'] == 'ZiG']
        if not post.empty:
        ...
        st.metric("Official/Parallel Spread", f"{spread:.1f}%",
                  delta="Narrowing ✅" if spread < 30 else "Wide ⚠️")

    st.subheader("💰 Asset Value Erosion / Growth Calculator")
    col_c1, col_c2 = st.columns(2)
    with col_c1:
        asset_value = st.number_input("Asset Value (ZiG)", min_value=0.0,
                                      value=100000.0, step=10000.0, key="erosion_val")
        years = st.slider("Years", 1, 10, 5, key="erosion_yrs")
    with col_c2:
        inf_scenario = st.selectbox("Inflation Scenario",
            ['Optimistic (5%)', 'Base Case (8.5%)', 'Pessimistic (12%)'])
        sc_rates = {'Optimistic (5%)': 5, 'Base Case (8.5%)': 8.5, 'Pessimistic (12%)': 12}
        sc_rate = sc_rates[inf_scenario]
        future_val = asset_value * (1 + sc_rate / 100) ** years
        ...
        real_val = asset_value  # constant purchasing power
        st.metric("Real Value (Purchasing Power)", f"ZiG {real_val:,.2f}",
                  delta="Policy covers real losses ✅", delta_color="normal")
...
with tab6:
    ...
    with col1:
        ...
        bars = ax.bar(trust_df['Period'], trust_df['Trust'], color=bar_colors, edgecolor='white',
        ...
        for b in bars:
            h = b.get_height()
            ax.text(b.get_x() + b.get_width() / 2., h + 2, f'{h}%',
        ...
        st.pyplot(fig)
        plt.close()
        st.success(f"✅ Trust has increased by **{85-35}%** since pre-ZiG era")
    ...
    with col2:
        ...
        if st.session_state.clients:
            ...
            st.metric("Average Client Trust", f"{avg_trust:.1f}%",
                      delta=f"+{avg_trust-35:.1f}% vs Pre-ZiG")
            fig2, ax2 = plt.subplots(figsize=(7, 4))
            ax2.hist(trust_scores, bins=10, range=(0, 100), color='#2e7d32',
                     alpha=0.75, edgecolor='white')
            ax2.axvline(x=avg_trust, color='#FFD700', linestyle='--',
                        linewidth=2.5, label=f'Average: {avg_trust:.0f}%')
            ...
            if at_risk:
                st.warning(f"⚠️ {at_risk} client(s) have low trust (<60%)")
            else:
...
with tab7:
    ...
    col3.metric("Solvency Ratio", f"{solvency:.1f}%",
                delta="✅ RBZ Compliant" if solvency >= 150 else "⚠️ Below Minimum",
    ...
    if total_risk > 0:
        ...
        ax.axvline(x=total_risk * 1.5, color='#ff9800', linestyle='--',
                   linewidth=2, label='RBZ Minimum (150%)')
    ax.set_xlabel('Amount (ZiG)')
    ...
    if not st.session_state.assets.empty:
        ...
        class_grp['Risk_Capital'] = class_grp.apply(
            ...
            axis=1)
        st.dataframe(class_grp.rename(columns={'RBZ_Class': 'Class', 'Total_Value': 'Total Value (ZiG)',
                                                 'Risk_Capital': 'Risk Capital Required (ZiG)'}),
                     use_container_width=True, hide_index=True)
        st.metric("Total Risk-Based Capital Required",
                  f"ZiG {class_grp['Risk_Capital'].sum():,.0f}")

    st.subheader("Capital Injection")
    col_c1, col_c2 = st.columns([2, 1])
    with col_c1:
        add_cap = st.number_input("Additional Capital (ZiG)", min_value=0.0,
...
with tab8:
    ...
    with col1:
        ...
        sc_params = {
            'Base Case (8.5% Inflation)': {'inflation': 8.5, 'growth': 10, 'claims_factor': 1.0, 'color': 'blue'},
            'Optimistic (5% Inflation)': {'inflation': 5, 'growth': 15, 'claims_factor': 0.7, 'color': 'green'},
            'Pessimistic (12% Inflation)': {'inflation': 12, 'growth': 8, 'claims_factor': 1.3, 'color': 'red'},
        }
        ...
        claims = [pr * 0.6 * p['claims_factor'] * (1 + p['inflation'] / 100) for pr in premiums]
        expenses = [pr * 0.2 for pr in premiums]
        profits = [pr - cl - ex for pr, cl, ex in zip(premiums, claims, expenses)]

        results = pd.DataFrame({'Year': years_list, 'Premiums': premiums,
                                 'Claims': claims, 'Expenses': expenses, 'Profit': profits})
        ...
        col_m1.metric("Projected Total Profit", f"ZiG {total_profit:,.0f}")
    ...
    with col2:
        ...
        ax.plot(years_list, claims, 'r-s', linewidth=2, markersize=5, label='Claims')
...
with tab9:
    ...
    with col1:
        ...
        **Economic Stabilization:**
        - ✅ Inflation: 500%+ → **8.5%** (single digit)
        - ✅ Exchange Rate: Stable at **26.90 ZiG/USD**
        - ✅ Insurance Penetration: **8% → 15%** (recovering)
        - ✅ Public Trust Score: **35% → 85%**
        ...
        **RBZ Key Data:**
        ...
        if investment > 0:
            ...
            st.info(f"⏱️ Payback Period: **{payback:.1f} months**")
...
<div style='text-align:center; color:#555; padding:1.5rem;
    ...
    <p style='font-size:1.2rem; font-weight:bold; color:#006400; margin:0;'>
        🛡️ RiskShield Zimbabwe
    </p>
    <p style='font-size:0.88rem; margin:0.3rem 0; color:#444;'>
        Complete Insurance Management System &nbsp;|&nbsp; Historical Currency Tracking 2009–2026 &nbsp;|&nbsp; v7.0
    </p>
    ...
    <p style='font-size:0.78rem; color:#999; margin:0.2rem 0;'>
        1 USD = {rates['official_rate']:.2f} ZiG &nbsp;|&nbsp;
        Inflation: {rates['inflation']}% &nbsp;|&nbsp;
        ZiG Stability: {rates['days_stable']} days &nbsp;|&nbsp;
        © 2026 RiskShield Zimbabwe
    </p>
</div>
""", unsafe_allow_html=True)
